...
//			self.contentSize = [_attributedTextContentView intrinsicContentSize];
			self.contentSize = CGSizeMake([_attributedTextContentView intrinsicContentSize].width, [_attributedTextContentView intrinsicContentSize].height + self.headerView.frame.size.height);
		}
	});
}
...
- (DTAttributedTextContentView *)attributedTextContentView
{
	if (!_attributedTextContentView)
	{
		...
		CGRect realRect;
		if (self.headerView) {
			realRect = CGRectMake(self.bounds.origin.x+8.0f, self.bounds.origin.y+self.headerView.frame.size.height, self.bounds.size.width-15.0f, self.bounds.size.height-self.headerView.frame.size.height);
		}else{
			realRect = CGRectMake(self.bounds.origin.x+8.0f, self.bounds.origin.y, self.bounds.size.width-15.0f,self.bounds.size.height);
		}
		[self addSubview:self.headerView];
		
//		CGRect frame = UIEdgeInsetsInsetRect(self.bounds, self.contentInset);
		CGRect frame = UIEdgeInsetsInsetRect(realRect, self.contentInset);
...
- (void)setHeaderView:(UIView *)headerView
{
	...
	{
		CGRect frame = _attributedTextContentView.frame;
		...
		self.contentSize = CGSizeMake(_attributedTextContentView.frame.size.width, _attributedTextContentView.frame.size.height+_headerView.frame.size.height);
		frame = _attributedTextContentView.frame;
		frame.origin.y += _headerView.frame.size.height;
		_attributedTextContentView.frame = frame;
	}
}
